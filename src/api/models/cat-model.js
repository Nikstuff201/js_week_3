...
const findCatById = async (id) => {
  const [rows] = await promisePool.execute('SELECT wsk_cats.cat_id, wsk_cats.cat_name, wsk_cats.weight, wsk_cats.owner AS owner_Id, wsk_users.username AS owner FROM wsk_cats INNER JOIN wsk_users ON wsk_cats.owner=wsk_users.user_id WHERE cat_id = ?', [id]);
  console.log('rows', rows);
  if (rows.length === 0) {
    return false;
  }
